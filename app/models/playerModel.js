...
exports.selectPlayerList = function (clubId, callback) {
    db.pool.acquire(function (err, conn) {
        if(err) console.error('db - err : ', err);
        conn.query('select p.playerId, u.userId, concat(u.lastName, u.firstName)playerName, DATE_FORMAT(u.birthday, "%Y/%m/%d") birthday, u.email, p.squadNumber, p.position, p.matchPosition, p.status, p.orderNumber, p.transfer from player p left outer join user u on u.userId = p.userId where p.clubId = ? order by p.squadNumber', [clubId], function(err, playerList) {
            if(err) console.error('err : ', err);
    ...
    db.pool.acquire(function (err, conn){
        ...
        conn.query('insert into player (userId, clubId, squadNumber, position, matchPosition, orderNumber, status, transfer) values (?, ?, ?, ?, ?, ?, ?, ?)', data, function(err, result) {
            ...
            if (result.affectedRows == 1) {
                ...
                return console.lor('fail insert.....   ', result);
            }
        });
        db.pool.release(conn);
    });
