...
exports.selectPlayer = function (email, callback) {
    db.pool.acquire(function (err, conn) {
        ...
        conn.query('select u.userId, concat(u.lastName, u.firstName)playerName, DATE_FORMAT(u.birthday, "%Y/%m/%d") birthday, u.email, u.password, p.playerId, p.squadNumber, p.position, p.matchPosition, p.status, p.transferStatus, p.transfer, p.clubId, c.leaderId, c.leagueId, l.community, l.season, t.teamId, t.teamName, te.teamName createTeamName, te.createId, te.teamId createTeamId from user u left outer join player p on u.userId = p.userId left outer join club c on p.clubId = c.clubId left outer join team t on c.teamId = t.teamId left outer join team te on u.userId = te.createId left outer join league l on l.leagueId = c.leagueId where u.email = ? group by u.userId', [email], function(err, arrPlayer) {
...
exports.selectPlayerList = function (clubId, callback) {
    db.pool.acquire(function (err, conn) {
        if(err) console.error('db - err : ', err);
        conn.query('select p.playerId, u.userId, concat(u.lastName, u.firstName)playerName, DATE_FORMAT(u.birthday, "%Y/%m/%d") birthday, u.email, p.squadNumber, p.position, p.matchPosition, p.status, p.orderNumber, p.transferStatus, p.transfer from player p left outer join user u on u.userId = p.userId where p.clubId = ? order by p.squadNumber', [clubId], function(err, playerList) {
            if(err) console.error('err : ', err);
...
exports.selectJoinedLeagues = function (userId, callback){
    db.pool.acquire(function (err, conn){
        ...
        // leagueStatus -> before, playing, end로 나뉨
